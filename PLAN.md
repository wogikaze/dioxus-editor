...
2. **core::selection / caret**: キャレット位置・選択範囲の管理（ノードID + オフセット）、行頭/行末/単語単位移動、範囲拡張/縮小。詳細手順は `docs/dev/selection.md` にまとめ、SelectionRange を中心に UI/Undo へ共有する。
3. **core::history**: Undo-tree を保持する履歴管理。分岐切替を許容し、構造変更も完全復元。初期は Document スナップショットを保存し、巨大データでは `Arc` 共有や差分ストレージへ切り替える（`docs/dev/09-undo-tree-and-history.md` の方針に従う）。
...
- 折り畳みは「可視ノードリスト」を生成して親が折り畳みなら子をスキップ。`ctrl+[number]` で閾値より深いノードに折り畳みフラグを設定。巨大ドキュメントでは `Document` のバージョン + 折り畳み世代に依存した `use_memo` キャッシュで可視リストを再利用し、毎フレームの全走査を避ける。
...
1. **基盤実装**: ドキュメントモデル、Undo-tree の骨格、可視ノード生成を実装。キーマップ/コマンドレイヤを整備。
2. **選択モデル**: `docs/dev/selection.md` の SelectionRange/CaretPosition を導入し、複数行選択・Shift 操作・Undo 復元までデータ構造を固める。
3. **編集・構造操作**: 改行・削除・インデント/デインデント・行移動/複製・折り畳み（クリック/ショートカット）を SelectionRange ベースで接続し、E-101〜E-105, E-201〜E-207 を動かす。
4. **記法レンダリング**: インライン装飾/リンク/タグ/チェックボックス、ブロック（コード/テーブル/LaTeX）、メディア埋め込みを追加し、N/L/BK 系テストを通す。
5. **入力補助と検索**: 括弧補完、リンク/タグ補完、検索窓、コマンドパレット、IME ガードを実装し、A/E-401〜E-404, E-701〜E-703 を通す。
6. **Undo-tree/履歴強化**: 分岐の可視化・切替、構造変更の復元精度を高め、E-601〜E-603 を検証。
7. **E2E/パフォーマンス**: TEST_SPEC.md ベースの E2E を Playwright/Cypress で自動化し、巨大データと高速入力の耐性を測定。
8. **埋め込み I/F と仕上げ**: 外部 I/F/テーマの整理、アクセシビリティ/フォーカスリング調整、ドキュメント更新。
