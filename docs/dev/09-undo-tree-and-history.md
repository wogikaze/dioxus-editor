...
- **メモリ戦略:** 初期実装ではスナップショットをそのまま `Document` ごとコピーしてもよいが、長時間編集や E-801/E-802 レベルのデータではメモリが膨らむ。`Arc<Line>` 等で行実体を共有する、もしくは「構造差分（パッチ）」を保持するバリアントを用意し、タイミングをみて切り替えられるようにする。
