...
| ID | テスト項目 | 手順 | 期待される動作 |
|:---|:---|:---|:---|
| **M-01** | 行移動 (Ctrl+Up/Down) | `Ctrl+Up` / `Down` を押下 | 行が上下に入れ替わること。 |
| **M-02** | 構造移動 (Alt+Up/Down) | 子要素を持つ行で `Alt+Up` / `Down` | 子要素を連れたまま（構造を維持して）兄弟要素間で移動すること。 |
| **M-03** | インデント移動 (Ctrl+Left/Right)| `Ctrl+Right` / `Left` を押下 | インデントレベルが変更されること（Tab操作と同等）。 |
...
| **A-11** | ノード間移動 | `Alt+Left` / `Right` | 親ノードへ移動 / 最初の子ノードへ移動すること。 |
...
| **E-206** | 折りたたみ行の改行 | 折りたたまれた親行の**行末**で`Enter` | 折りたたみを展開し、親行の次に行ができ、子要素は改行後の子要素に移る。 |
